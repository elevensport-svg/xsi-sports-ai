...
type StabilityScenarioResult = {
  label: string;
  overThreshold1: number;
  underThreshold1: number;
  overThreshold2: number;
  underThreshold2: number;
  bets: number;
  wins: number;
  losses: number;
  winRate: number;
...
const STABILITY_SCENARIOS: StabilityScenario[] = [
  { label: "OVER -2%", overDelta: -0.02, underDelta: 0 },
  { label: "BASE", overDelta: 0, underDelta: 0 },
  { label: "OVER +2%", overDelta: 0.02, underDelta: 0 },
  { label: "UNDER -2%", overDelta: 0, underDelta: -0.02 },
  { label: "UNDER +2%", overDelta: 0, underDelta: 0.02 },
  { label: "BOTH -2%", overDelta: -0.02, underDelta: -0.02 },
  { label: "BOTH +2%", overDelta: 0.02, underDelta: 0.02 },
];

function clampThreshold(value: number) {
  return Math.max(0.5, Math.min(0.75, round(value, 2)));
}

function combineTwoOOS(
  first: EvaluationResult,
  second: EvaluationResult,
) {
  const bets = first.bets + second.bets;
  const wins = first.wins + second.wins;
  const losses = first.losses + second.losses;

  const overBets = first.overBets + second.overBets;
  const overWins = first.overWins + second.overWins;

  const underBets = first.underBets + second.underBets;
  const underWins = first.underWins + second.underWins;
...
function classifyStability({
  ...
  baseWinRate: number;
  scenarioResults: StabilityScenarioResult[];
  season1WinRate: number;
  season2WinRate: number;
  baseBets: number;
  baseOverBets: number;
  baseUnderBets: number;
}) {
  const nonBase = scenarioResults.filter(
    (row) => row.label !== "BASE",
  );

  const worstNearbyWinRate =
    nonBase.length > 0
      ? Math.min(...nonBase.map((row) => row.winRate))
      : baseWinRate;

  const bestNearbyWinRate =
    nonBase.length > 0
      ? Math.max(...nonBase.map((row) => row.winRate))
      : baseWinRate;

  const nearbySpread =
    round(bestNearbyWinRate - worstNearbyWinRate, 1);

  const positiveSeasons =
    Number(season1WinRate >= 52.4) +
    Number(season2WinRate >= 52.4);
  ...
  const robustNeighborhood =
    worstNearbyWinRate >= 52.4 &&
    nearbySpread <= 8;
  ...
  return {
    ...
    criteria: {
      pass: {
        baseWinRateAtLeast: 55,
        betsAtLeast: 250,
        overBetsAtLeast: 75,
        underBetsAtLeast: 75,
        nearbyWorstAtLeast: 52.4,
        nearbySpreadAtMost: 8,
        positiveOOSSeasonsRequired: 2,
      },
      note:
        "V6 is a stability diagnostic. These cutoffs are validation rules, not newly optimized betting parameters.",
    },
  };
}
...
export async function GET() {
  try {
    ...
    console.log(
      "🏆 XSI TOTALS V6 STABILITY VALIDATION",
    );
    console.log(
      "========================================",
    );
    ...
    /* =========================
       V6 Stability Validation

       Freeze the V5-selected thresholds for each
       walk-forward fold, then perturb them +/- 2%.
       No scenario is used to re-select the model.
    ========================= */

    const stabilityResults: StabilityScenarioResult[] =
      STABILITY_SCENARIOS.map((scenario) => {
        const over1 = clampThreshold(
          overThreshold1 + scenario.overDelta,
        );
        const under1 = clampThreshold(
          underThreshold1 + scenario.underDelta,
        );

        const over2 = clampThreshold(
          overThreshold2 + scenario.overDelta,
        );
        const under2 = clampThreshold(
          underThreshold2 + scenario.underDelta,
        );

        const fold1 = evaluateSeason({
          baseHistory: season2324,
          targetMatches: season2425,
          overThreshold: over1,
          underThreshold: under1,
        });

        const fold2 = evaluateSeason({
          baseHistory: training2,
          targetMatches: season2526,
          overThreshold: over2,
          underThreshold: under2,
        });

        const combined = combineTwoOOS(fold1, fold2);

        return {
          label: scenario.label,
          overThreshold1: over1,
          underThreshold1: under1,
          overThreshold2: over2,
          underThreshold2: under2,
          ...combined,
        };
      });

    const stability = classifyStability({
      baseWinRate: combinedWinRate,
      scenarioResults: stabilityResults,
      season1WinRate: validation2425.winRate,
      season2WinRate: validation2526.winRate,
      baseBets: combinedBets,
      baseOverBets: combinedOverBets,
      baseUnderBets: combinedUnderBets,
    });
    ...
    console.log("🧪 V6 THRESHOLD STABILITY");
    console.log(
      "========================================",
    );

    for (const scenario of stabilityResults) {
      console.log(
        `${scenario.label.padEnd(10)} | Bets ${String(
          scenario.bets,
        ).padStart(3)} | Win ${scenario.winRate}% | OVER ${
          scenario.overWinRate
        }% | UNDER ${scenario.underWinRate}%`,
      );
    }

    console.log("----------------------------------------");
    console.log(
      `📉 Nearby Worst：${stability.worstNearbyWinRate}%`,
    );
    console.log(
      `📈 Nearby Best ：${stability.bestNearbyWinRate}%`,
    );
    console.log(
      `↔️ Nearby Spread：${stability.nearbySpread}%`,
    );
    console.log(
      `📅 Positive OOS Seasons：${stability.positiveSeasons}/2`,
    );
    console.log(
      `🧾 Sample Check：${stability.enoughSample ? "PASS" : "FAIL"}`,
    );
    console.log(
      `🧱 Neighborhood Check：${
        stability.robustNeighborhood ? "PASS" : "FAIL"
      }`,
    );
    console.log(
      `🏁 FINAL VERDICT：${stability.verdict}`,
    );
    ...
    return NextResponse.json({
      success: true,
      model:
        "XSI Football Totals V6 Stability Validation",
      league: LEAGUE,
      line: TOTAL_LINE,
