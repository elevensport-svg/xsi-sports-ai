...
/* ==========================================
   Historical Bundesliga XSI Probability V2
...
/* ==========================================
   ...
   預覽：
   /api/football/bundesliga-history-xsi-sync

   先跑 20 場：
   /api/football/bundesliga-history-xsi-sync?confirm=1&limit=20

   全部：
   /api/football/bundesliga-history-xsi-sync?confirm=1

   force=1 可重算：
   /api/football/bundesliga-history-xsi-sync?confirm=1&force=1
========================================== */
  ...
  try {
      ...
      return NextResponse.json(
        {
          ...
          message:
            "找不到德甲 2025/26 歷史資料，請先執行 bundesliga-history-sync。",
        },
        {
          status:
            404,
    ...
    for (
      ...
      try {
        ...
        console.log(
          `⚽ Bundesliga Historical XSI ${index + 1}/${targetRows.length}｜${row.away_team} @ ${row.home_team}｜Market H/D/A ${market.home}/${market.draw}/${market.away}｜XSI ${xsi.home}/${xsi.draw}/${xsi.away}`,
        );
      } catch (
        error
      ) {
        ...
        console.error(
          `❌ Bundesliga Historical XSI ${row.external_id} 失敗：`,
          error,
        );
      }
    }
    ...
    console.log(
      "🇩🇪 Bundesliga Historical XSI Sync 完成",
    );
    ...
    console.error(
      "❌ Bundesliga history-xsi-sync Error：",
      error,
    );
