...
export async function GET(
  ...
  try {
    ...
    if (error) {
      throw new Error(
        `讀取 Bundesliga Walk Forward 資料失敗：${error.message}`,
      );
    }
    ...
    console.log(
      "🇩🇪 BUNDESLIGA WALK FORWARD COMPLETE",
    );
    ...
    return NextResponse.json({
      ...
      conclusion:
        overallXsiAccuracy >
        overallMarketAccuracy
          ? "Bundesliga Walk-Forward XSI 整體優於 Market baseline，可繼續做 Value / ROI 驗證。"
          : "Bundesliga Walk-Forward XSI 未能整體優於 Market baseline，不建議直接把參數視為正式優勢。",
    });
  } catch (
    error
  ) {
    console.error(
      "❌ Bundesliga Walk Forward Error：",
      error,
    );
