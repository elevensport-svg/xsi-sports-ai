...
/* ==========================================
   Historical EPL XSI Probability V2
   ...
   目的不是直接把這組參數當 EPL 最佳參數，
   而是先建立 EPL 歷史 XSI baseline，
   後面再用 EPL 380 場調參。
========================================== */
...
/* ==========================================
   ...
   預覽：
   /api/football/epl-history-xsi-sync

   先跑 20 場：
   /api/football/epl-history-xsi-sync?confirm=1&limit=20

   全部：
   /api/football/epl-history-xsi-sync?confirm=1

   force=1 可重算：
   /api/football/epl-history-xsi-sync?confirm=1&force=1
========================================== */
  ...
  try {
      ...
      return NextResponse.json(
        {
          ...
          message:
            "找不到 EPL 2025/26 歷史資料，請先執行 epl-history-sync。",
        },
        {
          status:
            404,
    ...
    for (
      ...
      try {
        ...
        console.log(
          `⚽ EPL Historical XSI ${index + 1}/${targetRows.length}｜${row.away_team} @ ${row.home_team}｜Market H/D/A ${market.home}/${market.draw}/${market.away}｜XSI ${xsi.home}/${xsi.draw}/${xsi.away}`,
        );
      } catch (
        error
      ) {
