 ...
 * ...
 *
 * 每個 Fold 都只用當時以前的資料重新選參數。
 *
 * URL:
 * /api/football/epl-walk-forward
 *
 * 可調：
...
export async function GET(
  ...
  try {
    ...
    if (error) {
      throw new Error(
        `讀取 EPL Walk Forward 資料失敗：${error.message}`,
      );
    }
    ...
    return NextResponse.json({
      ...
      conclusion:
        overallXsiAccuracy >
        overallMarketAccuracy
          ? "Walk-Forward XSI 整體優於 Market baseline；下一步仍應檢查各 Fold 穩定性與參數一致性。"
          : "Walk-Forward XSI 未能整體優於 Market baseline；不建議直接將 EPL 專屬參數接入正式模型。",
    });
  } catch (error) {
    console.error(
      "❌ EPL Walk Forward Error：",
      error,
    );
