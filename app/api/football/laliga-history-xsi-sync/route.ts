...
/* ==========================================
   Historical La Liga XSI Probability V2
...
/* ==========================================
   ...
   預覽：
   /api/football/laliga-history-xsi-sync

   先跑 20 場：
   /api/football/laliga-history-xsi-sync?confirm=1&limit=20

   全部：
   /api/football/laliga-history-xsi-sync?confirm=1

   force=1 可重算：
   /api/football/laliga-history-xsi-sync?confirm=1&force=1
========================================== */
  ...
  try {
      ...
      return NextResponse.json(
        {
          ...
          message:
            "找不到西甲 2025/26 歷史資料，請先確認 football_match_history 已有西甲資料。",
        },
        {
          status:
            404,
    ...
    for (
      ...
      try {
        ...
        console.log(
          `⚽ La Liga Historical XSI ${index + 1}/${targetRows.length}｜${row.away_team} @ ${row.home_team}｜Market H/D/A ${market.home}/${market.draw}/${market.away}｜XSI ${xsi.home}/${xsi.draw}/${xsi.away}`,
        );
      } catch (
        error
      ) {
        ...
        console.error(
          `❌ EPL Historical XSI ${row.external_id} 失敗：`,
          error,
        );
      }
    }
    ...
    console.log(
      "🇪🇸 La Liga Historical XSI Sync 完成",
    );
    ...
    console.error(
      "❌ La Liga history-xsi-sync Error：",
      error,
    );
