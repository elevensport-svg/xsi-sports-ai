...
/* ==========================================
   Historical Ligue 1 XSI Probability V2
...
/* ==========================================
   ...
   預覽：
   /api/football/ligue1-history-xsi-sync

   先跑 20 場：
   /api/football/ligue1-history-xsi-sync?confirm=1&limit=20

   全部：
   /api/football/ligue1-history-xsi-sync?confirm=1

   force=1 可重算：
   /api/football/ligue1-history-xsi-sync?confirm=1&force=1
========================================== */
  ...
  try {
      ...
      return NextResponse.json(
        {
          ...
          message:
            "找不到法甲 2025/26 歷史資料，請先執行 ligue1-history-sync。",
        },
        {
          status:
            404,
    ...
    for (
      ...
      try {
        ...
        console.log(
          `⚽ Ligue 1 Historical XSI ${index + 1}/${targetRows.length}｜${row.away_team} @ ${row.home_team}｜Market H/D/A ${market.home}/${market.draw}/${market.away}｜XSI ${xsi.home}/${xsi.draw}/${xsi.away}`,
        );
      } catch (
        error
      ) {
        ...
        console.error(
          `❌ Ligue 1 Historical XSI ${row.external_id} 失敗：`,
          error,
        );
      }
    }
    ...
    console.log(
      "🇫🇷 Ligue 1 Historical XSI Sync 完成",
    );
    ...
    console.error(
      "❌ Ligue 1 history-xsi-sync Error：",
      error,
    );
