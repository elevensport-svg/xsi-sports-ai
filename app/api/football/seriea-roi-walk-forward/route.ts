...
type FoldResult = {
  ...
  xsiOos: StrategyResult | null;
  marketBaselineOos: StrategyResult | null;
};
...
function odds(row: HistoryRow, side: Side) {
...
function valueGap(row: HistoryRow, side: Side) {
  const x = probability(row, side, "xsi");
  const m = probability(row, side, "market");
...
function evaluate(
  ...
  for (const row of rows) {
    ...
    const selected =
      mode === "xsi" ? xsiLeader(row) : marketLeader(row);

    if (selected !== strategy.side) continue;
    ...
    const selectedOdds = odds(row, strategy.side);
    ...
    if (actual(row) === strategy.side) {
      wins += 1;
      totalReturn += selectedOdds;
      losing = 0;
    } else {
      losing += 1;
      maxLosingStreak = Math.max(maxLosingStreak, losing);
    }
  }

  const losses = games - wins;
  const profit = totalReturn - games;
  ...
  return {
    ...
    wins,
    losses,
    hitRate: games ? round((wins / games) * 100) : 0,
    totalStake: games,
...
function chooseStrategy(
  training: HistoryRow[],
  minTrainingBets: number,
) {
  const candidates = strategies()
    .map((strategy) => evaluate(training, strategy, "xsi"))
    .filter((r) => r.games >= minTrainingBets)
    .sort((a, b) => {
...
function aggregate(
  results: Array<StrategyResult | null>,
) {
  const valid = results.filter(
    (r): r is StrategyResult => r !== null,
  );

  const games = valid.reduce((sum, r) => sum + r.games, 0);
  const wins = valid.reduce((sum, r) => sum + r.wins, 0);
  const totalReturn = valid.reduce(
    (sum, r) => sum + r.totalReturn,
    0,
  );
  const profit = totalReturn - games;
...
export async function GET(request: Request) {
  try {
    ...
    if (error) {
      throw new Error(
        `讀取 Serie A ROI Walk-Forward 資料失敗：${error.message}`,
      );
    }
    ...
    while (cursor < rows.length) {
      ...
      let xsiOos: StrategyResult | null = null;
      let marketBaselineOos: StrategyResult | null = null;

      if (selected) {
        xsiOos = evaluate(test, selected, "xsi");

        /*
         * 公平對照：
         * 同一 side + 同一 odds range，
         * 但 Market 不使用 XSI Value Gap。
         */
        marketBaselineOos = evaluate(
          test,
          {
            side: selected.side,
            threshold: 0,
            oddsMin: selected.oddsMin,
            oddsMax: selected.oddsMax,
            oddsRange: selected.oddsRange,
          },
          "market",
        );
      }
      ...
      folds.push({
        ...
        trainingEnd: dateOnly(
          training[training.length - 1].match_date,
        ),
    ...
    const xsiOverall = aggregate(
      folds.map((fold) => fold.xsiOos),
    );

    const marketOverall = aggregate(
      folds.map((fold) => fold.marketBaselineOos),
    );

    const selectedFrequency = new Map<
      string,
      { strategy: Strategy; count: number }
    >();

    for (const fold of folds) {
      if (!fold.selectedStrategy) continue;

      const strategy: Strategy = {
        side: fold.selectedStrategy.side,
        threshold: fold.selectedStrategy.threshold,
        oddsMin: fold.selectedStrategy.oddsMin,
        oddsMax: fold.selectedStrategy.oddsMax,
        oddsRange: fold.selectedStrategy.oddsRange,
      };

      const key = JSON.stringify(strategy);
      const current = selectedFrequency.get(key);

      if (current) {
        current.count += 1;
      } else {
        selectedFrequency.set(key, {
          strategy,
          count: 1,
        });
      }
    ...
    const mostSelectedStrategies = Array.from(
      selectedFrequency.values(),
    ).sort((a, b) => b.count - a.count);

    console.log("======================================");
    console.log("🇮🇹 SERIE A ROI WALK FORWARD COMPLETE");
    ...
    console.log(`XSI OOS Bets：${xsiOverall.bets}`);
    console.log(`XSI OOS Profit：${xsiOverall.profit}`);
    console.log(`XSI OOS ROI：${xsiOverall.roi}%`);
    console.log(`Market OOS Bets：${marketOverall.bets}`);
    console.log(`Market OOS Profit：${marketOverall.profit}`);
    console.log(`Market OOS ROI：${marketOverall.roi}%`);
    console.log("======================================");
    ...
    return NextResponse.json({
      ...
      methodology: {
        type: "expanding-window ROI walk-forward",
      ...
      overall: {
        xsi: xsiOverall,
        marketBaseline: marketOverall,
        roiDifference: round(
          xsiOverall.roi - marketOverall.roi,
        ),
      },
    ...
    console.error(
      "❌ Serie A ROI Walk-Forward Error：",
      error,
    );
