 ...
 *
 * URL:
 * /api/football/seriea-walk-forward
 *
 * 可調：
...
export async function GET(
  ...
  try {
    ...
    if (error) {
      throw new Error(
        `讀取 Serie A Walk Forward 資料失敗：${error.message}`,
      );
    }
    ...
    return NextResponse.json({
      ...
      conclusion:
        overallXsiAccuracy >
        overallMarketAccuracy
          ? "Serie A Walk-Forward XSI 整體優於 Market baseline，可繼續做 Value / ROI 驗證。"
          : "Serie A Walk-Forward XSI 未能整體優於 Market baseline，不建議直接把參數視為正式優勢。",
    });
  } catch (
    error
  ) {
    console.error(
      "❌ Serie A Walk Forward Error：",
      error,
    );
