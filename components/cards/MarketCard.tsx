...
function getPriority(bookmaker: MarketBookmaker): number {
  const priority = BOOKMAKER_PRIORITY.indexOf(bookmaker.key);
...
function buildBookmakerRows(
  ...
  return [...market.bookmakers]
    ...
    .map((bookmaker) => ({
      bookmaker,
      awayMoneyline: findTeamOutcome(
        bookmaker.moneyline,
        market.awayTeam,
      ),
      homeMoneyline: findTeamOutcome(
        bookmaker.moneyline,
        market.homeTeam,
      ),
      awaySpread: findTeamOutcome(
        bookmaker.spreads,
        market.awayTeam,
      ),
      homeSpread: findTeamOutcome(
        bookmaker.spreads,
        market.homeTeam,
      ),
      over: findTotalOutcome(bookmaker.totals, "over"),
      under: findTotalOutcome(bookmaker.totals, "under"),
    }));
}

function getBestPrice(
  outcomes: Array<MarketOutcome | null>,
): number | null {
...
export default function MarketCard({
  ...
  const rows = buildBookmakerRows(market);

  const bestAwayMoneyline = getBestPrice(
    rows.map((row) => row.awayMoneyline),
  );

  const bestHomeMoneyline = getBestPrice(
    rows.map((row) => row.homeMoneyline),
  );
    ...
    <section className="mt-10 rounded-3xl border border-yellow-500/20 bg-zinc-950 p-6 md:p-8">
      <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
        ...
        <div className="rounded-xl bg-zinc-900 px-4 py-3 md:text-right">
          ...
          <p className="mt-1 text-sm font-bold text-zinc-300">
            {formatTaiwanTime(market.commenceTime)}
          </p>
        </div>
      </div>
      ...
      <div className="mt-6 grid gap-4 md:grid-cols-3">
        <ConsensusCard
          ...
          awayValue={market.consensus.awayMoneyline}
          homeValue={market.consensus.homeMoneyline}
          formatValue={formatAmericanOdds}
        />
        ...
        <ConsensusCard
          ...
          awayTeam={market.awayTeam}
          homeTeam={market.homeTeam}
          awayValue={market.consensus.awaySpread}
          homeValue={market.consensus.homeSpread}
          ...
          <table className="min-w-[1050px] w-full text-left">
            <thead className="bg-black/40 text-xs uppercase tracking-wider text-zinc-500">
              <tr>
                ...
                <th className="px-4 py-4">大分</th>
                <th className="px-4 py-4">小分</th>
            ...
            <tbody className="divide-y divide-zinc-800">
              {rows.map((row) => {
                const awayMoneylineIsBest =
                  row.awayMoneyline?.price ===
                  bestAwayMoneyline;

                const homeMoneylineIsBest =
                  row.homeMoneyline?.price ===
                  bestHomeMoneyline;
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-4 py-4">
                      <OddsValue
                        value={row.awayMoneyline?.price}
                        isBest={awayMoneylineIsBest}
                      />
                    </td>

                    <td className="px-4 py-4">
                      <OddsValue
                        value={row.homeMoneyline?.price}
                        isBest={homeMoneylineIsBest}
                      />
                    </td>

                    <td className="px-4 py-4">
                      <SpreadValue
                        outcome={row.awaySpread}
                      />
                    </td>

                    <td className="px-4 py-4">
                      <SpreadValue
                        outcome={row.homeSpread}
                      />
                    </td>
