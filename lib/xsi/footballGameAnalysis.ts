...
export async function calculateFootballGameAnalysis(
  ...
  let laLigaMarketSignal:
    | "STRONG"
    | "SECONDARY"
    | "NONE" =
    "NONE";

  let laLigaMarketSignalBonus =
    0;
  ...
  if (
    ...
    homeWinReasons.push(
      `西甲 Market Signal：強訊號（主勝賠率 ${homeWinOdds?.toFixed(
        2,
      )}）`,
    );
  } else if (
    ...
    homeWinReasons.push(
      `西甲 Market Signal：次強訊號（主勝賠率 ${homeWinOdds?.toFixed(
        2,
      )}）`,
    );
  }
